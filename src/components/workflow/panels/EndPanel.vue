<template>
  <div class="panel-container space-y-4">
    ...
    <div class="panel-section bg-gray-50 p-3 rounded-md">
...
onMounted(() => {
  title.value = props.node.data?.title || t("workflow.nodes.end");
...
watch(
  () => props.node,
  (newNode) => {
    title.value = newNode.data?.title || t("workflow.nodes.end");
