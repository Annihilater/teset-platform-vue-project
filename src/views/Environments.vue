...
const selectedEnvironment = ref<TestEnvironment | null>(null);
const environments = ref<TestEnvironment[]>([]);
const recentActivities = ref<EnvironmentActivity[]>([]);
...
// 获取健康状态文本
const getHealthText = (status: HealthStatus) => {
  switch (status) {
    case "healthy":
      return "健康";
    case "degraded":
...
// 获取状态样式类
const getStatusClass = (status: EnvironmentStatus) => {
  switch (status) {
    case "active":
